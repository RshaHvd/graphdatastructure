...
abstract class IndexNode[T: Ordering] extends TreeNode[T] {
  ...
  def findFirstChildIndexLessThan(t: T): Option[Int] = {
    val mayBeChildListIndex: Option[(T, Int)] = this.listOfNodes.zipWithIndex.find {
      case (tIdx, idx) => t < tIdx
    }
...
class DefaultLeafNode[T: Ordering, D](inputNodes: List[T] = Nil,
...
class BPlusTreeImpl[T: Ordering, D](override val fanout: Int) extends BPlusTree[T, D] {
  ...
  override def add(t: T, d: D) = {
    ...
    def addToThisTree(_thisTree: TreeNode[T]): TreeNode[T] = _thisTree match {
      ...
      case y: IndexNode[T] => {
        ...
        val (childNode, childNodeIndex): (TreeNode[_], Int) = if (mayBeChildIndexLessThan.isDefined) {
