...
trait LeafNode[T, D] extends TreeNode[T] {
  ...
  override def add(t: T): Unit =
    throw new RuntimeException(s"Leaf Nodes cannot be added without data. Invalid call.")
...
abstract class IndexNode[T: Ordering] extends TreeNode[T] {
  ...
  def findFirstChildIndexLessThan(t: T): Option[Int] = {
    val mayBeChildListIndex: Option[(T, Int)] = this.listOfNodes.zipWithIndex.find { case (tIdx, idx) =>
      t < tIdx
    }
...
class DefaultLeafNode[T: Ordering, D](inputNodes: List[T] = Nil, dataForNodes: Seq[(T, D)] = Seq.empty[(T, D)])
      ...
      _thisTree match {
        ...
        case x: LeafNode[T, D] =>
          ...
          if (x.listOfNodes.size < nodeElementsSize) {
            ...
            val newRoot = new DefaultIndexNode(mutable.ListBuffer(k), mutable.ListBuffer(List(n1, n2): _*))
            ...
            case y1: IndexNode[T] =>
              ...
              if (heightOfOriginalTree == heightOfUpdateTree)
                ...
                if (allN.size <= nodeElementsSize) {
                  ...
                  // join this to the parent y
                  val updatedR =
                    new DefaultIndexNode(mutable.ListBuffer(key), mutable.ListBuffer(List(leftNode, rightNode): _*))
...
object TreeOps {
  ...
  def treeHeight[T: Ordering, D](tree: TreeNode[T]): Int = {
    ...
    def internalHeight(thisTree: TreeNode[T], heightSoFar: Int): Int =
      thisTree match {
        ...
        case t2: IndexNode[T] => internalHeight(t2.children.head, heightSoFar + 1)
      }
  ...
  def addAndSplitIndexNodes[T: Ordering, D](
    ...
    val (allNodes, allChildren) =
      createAllNodesAndChildren(List(key), List(lChild, rChild), childPosition, indexNodeTree)
  ...
  def createRootAndIndexNode[T: Ordering, D](
    ...
    val leftIndexNode: TreeNode[T] =
      new DefaultIndexNode(mutable.ListBuffer(n1Nodes: _*), mutable.ListBuffer(node1Children: _*))

    val rightIndexNode: TreeNode[T] =
      new DefaultIndexNode(mutable.ListBuffer(n2Nodes: _*), mutable.ListBuffer(node2Children: _*))
